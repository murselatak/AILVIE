...
const[newMed,setNewMed]=useState({name:"",dose:"",time:"",alarmType:"both",count:30});
const WORD_TOPICS={
health:[
...
const hscore=(hd.pulse===0&&hd.weight===0)?0:Math.min(100,Math.round((hd.pulse>=60&&hd.pulse<=100?25:hd.pulse>0?10:0)+(bmi>=18.5&&bmi<25?25:bmi>0?10:0)+(hd.bpS>=90&&hd.bpS<=140?25:hd.bpS>0?10:0)+(medProg>50?25:medProg>0?15:0)));
...
8) İlaç etkileşimleri konusunda uyar.`,messages:history})});
  ...
  }catch(e){setChatM(p=>[...p,{role:"assistant",text:lang==="tr"?"Bağlantı hatası. İnternet bağlantınızı kontrol edin. 💙":"Connection error. Check your internet. 💙"}]);}
  setChatL(false);
};
...
const MicBtn=({onResult})=>{
  return <button onClick={()=>startVoice(onResult)} style={{background:isListen?`${dg}22`:`${ac}15`,border:`1px solid ${isListen?dg:bd}`,borderRadius:8,padding:"8px 12px",cursor:"pointer",fontSize:18,animation:isListen?"micPulse 2s infinite":"none"}}>{isListen?"🔴":"🎤"}</button>;
};
      ...
      {<div onClick={()=>goTo("medTime")} style={{...CS,cursor:"pointer",padding:"12px",border:`1px solid ${ac}33`,background:`${ac}06`}}>
        ...
        {nextMed?<div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:fs-3,color:mt}}>{pat.name?pat.name.split(" ")[0]+", ":""}{nextMed.name} {lang==="tr"?"saatin yaklaşıyor!":"time approaching!"}</div>
          </div>
...
const renderSettings=()=>{const s=settingsTab;const all=s==="all";return(<div style={{display:"flex",flexDirection:"column",gap:10}}>
  ...
  {(all||s==="subs")&&<div style={CS}><div style={{fontWeight:700,marginBottom:8}}>💎 {t.subscription}</div>
    {[{n:t.free,p:"$0",d:t.freePlan,c:sc,active:true},{n:t.premium,p:"$4.99/"+t.monthly,d:t.premPlan,c:ac,active:false},{n:t.enterprise,p:"$12.99/"+t.monthly,d:lang==="tr"?"Gelişmiş AI analiz, öncelikli destek, reklamsız deneyim":"Advanced AI analysis, priority support, ad-free",c:a2,active:false}].map(plan=>(<div key={plan.n} style={{padding:"8px 10px",borderRadius:10,border:`1px solid ${plan.c}33`,marginBottom:6,background:`${plan.c}08`}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontWeight:700,color:plan.c}}>{plan.n}</span><div style={{display:"flex",alignItems:"center",gap:6}}>{!plan.active&&<span style={{fontSize:fs-3,padding:"2px 8px",borderRadius:8,background:`${mt}22`,color:mt,fontWeight:600}}>{lang==="tr"?"Yakında":"Coming Soon"}</span>}<span style={{fontWeight:700}}>{plan.p}</span></div></div><div style={{fontSize:fs-2,color:mt,marginTop:2}}>{plan.d}</div>{plan.active&&<div style={{fontSize:fs-3,color:sc,marginTop:4,fontWeight:600}}>✓ {lang==="tr"?"Aktif Plan":"Active Plan"}</div>}</div>))}
  </div>}
...
const renderAdmin=()=>(<div style={{display:"flex",flexDirection:"column",gap:8,height:"100%"}}>
  ...
  <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:8,minHeight:180}}>
    ...
    {adminMsgs.map((m,i)=>(<div key={i} style={{...CS,maxWidth:"85%",alignSelf:m.from==="user"?"flex-end":"flex-start",background:m.from==="user"?`linear-gradient(135deg,#f59e0b,#f97316)`:cd,color:m.from==="user"?"#fff":tc,animation:"slideD .3s"}}><div style={{whiteSpace:"pre-wrap",fontSize:fs}}>{m.text}</div><div style={{fontSize:fs-3,color:m.from==="user"?"rgba(255,255,255,.7)":mt,marginTop:4}}>{m.time}</div></div>))}
  </div>
...
const nav2=[{key:"pCard",icon:"🪪",label:t.pCard},{key:"notes",icon:"📝",label:t.notes},{key:"community",icon:"👥",label:t.community},{key:"chat",icon:"🤖",label:t.chat},{key:"admin",icon:"💬",label:t.adminCh||"Destek"},{key:"settings",icon:"⚙️",label:t.settings}];
...
return (
  <div style={{width:"100%",maxWidth:480,margin:"0 auto",height:"100dvh",display:"flex",flexDirection:"column",overflow:"hidden",background:bg,fontSize:fs,color:tc,fontFamily:"'SF Pro Display',-apple-system,'Segoe UI',system-ui,sans-serif",direction:rtl?"rtl":"ltr",position:"relative"}}>
        ...
        {showMenu&&<>
          ...
          <div style={{position:"absolute",top:0,left:0,bottom:0,width:"75%",background:cd,zIndex:450,padding:"60px 20px 20px",overflowY:"auto",boxShadow:"4px 0 20px rgba(0,0,0,.3)"}}>

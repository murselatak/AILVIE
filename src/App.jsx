...
const sendChat=async(text)=>{
  ...
  try{
    ...
    const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:apiHeaders(apiKey),body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:`Sen AILVIE — dünya çapında hizmet veren, sıcak, şefkatli ve güvenilir bir KADIN sağlık asistanısın.${ctxStr}
ÖNEMLİ KURALLAR:
...
8) İlaç etkileşimleri konusunda uyar.`,messages:history})});
  ...
  }catch(e){setChatM(p=>[...p,{role:"assistant",text:lang==="tr"?(apiKey?"Üzgünüm, şu an yanıt veremiyorum. Lütfen internet bağlantınızı kontrol edip tekrar deneyin. 💙":"⚠️ AI Sohbet için API anahtarı gerekli.\n\n⚙️ Ayarlar → API Key bölümünden Anthropic API anahtarınızı girin.\n\n🔑 Anahtar almak için: console.anthropic.com"):(apiKey?"Sorry, I cannot respond right now. Please check your internet connection. 💙":"⚠️ API key required for AI Chat.\n\n⚙️ Go to Settings → API Key to enter your Anthropic API key.\n\n🔑 Get a key at: console.anthropic.com")}]);}
  setChatL(false);
};
...
const renderMeds=()=>(<div style={{display:"flex",flexDirection:"column",gap:10}}>
  ...
  {showAddMed&&<div style={{...CS,border:`2px solid ${ac}`}}>
    ...
    <div style={{display:"flex",gap:6,marginBottom:6,alignItems:"center"}}><span style={{fontSize:fs-2,color:mt}}>💊 {lang==="tr"?"Kutu adedi":"Box count"}:</span><input type="number" min="1" value={newMed.count} onChange={e=>setNewMed({...newMed,count:Number(e.target.value)})} style={{...IS,width:70,textAlign:"center",fontWeight:700}}/><span style={{fontSize:fs-2,color:mt,marginLeft:8}}>🔁 {lang==="tr"?"Günde":"Per day"}:</span><input type="number" min="1" max="6" value={newMed.timesPerDay||1} onChange={e=>setNewMed({...newMed,timesPerDay:Number(e.target.value)})} style={{...IS,width:50,textAlign:"center",fontWeight:700}}/></div>
    {(newMed.timesPerDay||1)>1&&<div style={{fontSize:fs-3,color:ac,marginBottom:6,padding:"4px 8px",background:`${ac}08`,borderRadius:6}}>💡 {lang==="tr"?`Günde ${newMed.timesPerDay} doz için: Bu ilacı ${newMed.timesPerDay} kez farklı saatlerle ekleyin.`:`For ${newMed.timesPerDay} doses/day: Add this med ${newMed.timesPerDay} times with different hours.`}</div>}
...
return(<div style={{display:"flex",flexDirection:"column",gap:10}}>
  ...
  {/* Health Score */}
  <div style={{...CS,textAlign:"center",padding:14,border:`1px solid ${ac}33`}}>
    <div style={{fontSize:fs-1,color:mt,marginBottom:4}}>{t.hScore}</div>
    <div style={{fontSize:fs+12,fontWeight:800,color:hscore>70?sc:hscore>40?ac:hscore>0?dg:mt}}>{hscore||"—"}</div>
    <div style={{fontSize:fs-3,color:mt}}>{lang==="tr"?"Referans: acibadem.com.tr":"Ref: acibadem.com.tr"}</div>
  </div>
</div>);};

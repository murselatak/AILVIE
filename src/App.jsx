...
const loadWeather=useCallback(()=>{ // request location + fetch Open-Meteo (no API key)
...
const renderHome=()=>{
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:8}}>
    ...
    <div style={{display:"flex",gap:8}}>
      <div onClick={()=>{if(weather&&weather.cur)setShowWx(true);else loadWeather();}} style={{...CS,flex:1,padding:"10px 12px",display:"flex",alignItems:"center",gap:9,cursor:"pointer"}}>
    ...
    {showWx&&<div style={{position:"fixed",inset:0,zIndex:360,background:"rgba(0,0,0,.55)",display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>setShowWx(false)}>
      <div onClick={e=>e.stopPropagation()} style={{background:cd,width:"100%",maxWidth:520,maxHeight:"82vh",overflow:"auto",borderRadius:"16px 16px 0 0",padding:16}}>
        ...
        </>:<div style={{color:mt,fontSize:fs-1,padding:"24px 0",textAlign:"center"}}>{weather&&weather.err?(<><div>{lang==="tr"?"Hava durumu için konum gerekiyor.":"Weather needs your location."}</div><button onClick={()=>loadWeather()} style={{marginTop:12,padding:"10px 18px",borderRadius:12,border:"none",background:ac,color:"#fff",fontWeight:700,fontSize:fs,cursor:"pointer"}}>📍 {lang==="tr"?"Konumu Etkinleştir":"Enable Location"}</button></>):(lang==="tr"?"Yükleniyor…":"Loading…")}</div>}
